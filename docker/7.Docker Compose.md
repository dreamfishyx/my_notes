...
3. 在Docker主机的 ~/ 目录中创建一个名称为 finance (金融)的目录。将上述项目打包为 jar 包，更名为 user_manager.jar 上传至该目录。然后在该目录中新建 Dockerfile 文件。
...
4. 在finance目录中新建一个文件compose.yml：
...
5. 运行 `docker compose config -q` 检查文件是否存在错误。没有错误后运行 `docker compose -f compose.yaml up -d` 构建并启动项目。<font color=red>顺便提一嘴，在修改后建议使用 `--build` 重新构建镜像，要不你就删除所以镜像。</font>
