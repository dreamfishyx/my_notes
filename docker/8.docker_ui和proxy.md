...
4. 安装:
   ...
   2. 在 cig 目录中创建 compose.yml，内容如下<font color=red>后面提供一个gpt优化的版本</font>：
      ...
      services:
        influxdb:
          ...
          ports:
            ...
            - "8086:8086" # 后台端口
          volumes:
            - ./data/influxdb:/var/lib/influxdb  # 官方镜像的正确挂载路径
      ...
      >
      > 2. 报错`Failed to create a Container Manager: mountpoint for cpu not found` 说明 cAdvisor 在尝试创建容器管理器时无法找到 `cpu` 的挂载点。
      >
      >    1. 检查系统是否支持 cgroup ：`mount | grep cgroup`。
      >
      >       ```bash
      >       ❯ mount | grep cgroup
      ...
      >       ```
      >
      >    2. 系统已经挂载了 `cgroup2`，而 cAdvisor 可能依赖的是 `cgroup v1`，因此它无法找到 `cpu` 的挂载点。这在运行 cAdvisor 时会导致 `Failed to create a Container Manager` 的错误。
      >
      >    3. 编辑 `sudo vim /etc/default/grub` 文件添加:`GRUB_CMDLINE_LINUX="cgroup_enable=memory swapaccount=0 systemd.unified_cgroup_hierarchy=0"`
      >
      >    4. 保存文件并更新 grub：`sudo update-grub`
      >
      >    5. 重启系统，重新启动项目。
      >
      >    6. 然而上述操作对于 wsl 似乎无效(找不到命令)。于是采用第二套方案，更换较新的 cAdvisor 。去官方镜像中心逛一圈发现：已弃用：不会推送新图像，请改用 `gcr.io/cadvisor/cadvisor`。
      >
      >    7. 听话直接改用最新镜像,但是随之而来的新问题也出现，无法访问 gcr.io 。这点通过配置 docker 代理解决。
      >
      >    8. 于是乎给出最新的 compose 文件:
      >
      >       ```yaml
      >       volumes:
      >         grafana_data: {}
      ...
      >       
      >         cadvisor:
      >           image: mirrorgooglecontainers/cadvisor
      >           container_name: my_cadvisor
      >           command: -storage_driver=influxdb -storage_driver_db=cadvisor -storage_driver_host=influxdb:8086 # 修改为服务名
      >           restart: always
      >           ports:
   ...
   3. 通过 `docker-compose up –d` 启动所有容器。显示报错`Failed to create a Container Manager: mountpoint for cpu not found` 说明 cAdvisor 在尝试创建容器管理器时无法找到 `cpu` 的挂载点。

      1. 

   4. 

5. 查看 cadvisor 页面：由于其完成的是原始数据收集功能，所以该页面在打开时会较慢，有一个收集、展示的过程。在浏览器输入[localhost:8080](http://localhost:8080/)即可访问。
...
3. 配置 docker 守护进程代理：
   ...
   2. 手动创建 Docker 的 systemd 服务配置目录(若不存在), 然后创建或编辑 `http-proxy.conf` 文件,添加如下内容(若是自己的镜像加速在国内可以排除，不走代理):<font color=red>需要说明的是，上述使用第二种方式，直接依照实际情况配置代理 ip 和 port。</font>
      ...
      Environment="NO_PROXY=localhost,127.0.0.1,docker.dreamsea.top"
      ```

   3. 但是对于第一种方式，由于 systemd 无法访问环境变量，这里我们做一些小处理：配置`http-proxy.conf` 文件:
      ...
      > EnvironmentFile 使用 `-` 在目录前，作用是忽略文件不存在。

   4. 启动时生成配置文件, `vim ~/.zshrc`(不要忘记 `source ~/.zshrc`):
...
`systemd` 的 `EnvironmentFile` 只会影响服务的环境，但不会直接反映在 `systemctl show --property=Environment` 输出中。你看到的输出仅反映 `docker` 服务的系统级别环境变量，而不是通过 `EnvironmentFile` 加载的环境变量。

https://liuzhicong.cn/index.php/guide/systemd-environment-variable.html

https://yufanonsoftware.me/posts/manage-system-and-application-by-systemd.html
