...
4. 开启集群的自动锁定功能:
   ...
   2. manager 节点通过 `docker swarm update --autolock=true` 命令可以开启当前 swarm 集群的自动锁定功能。
      ...
      > 关闭设置成false就行，`docker swarm update --autolock=false`。
...
9. 要想解锁，我们需要在 docker3 中运行 `docker swarm unlock` 命令，解锁 swarm, 该过程需要输入之前的秘钥。解锁完成后，节点就会自动加入集群中。
   ...
   > 需要说明:在 Docker Swarm 中，用于加密和解密的公钥与私钥存储在各个节点的内存中。当节点的 Docker 守护进程重启时，这些密钥会丢失。如果在同一时间所有节点都重启或宕机，密钥将不可用，即使你记住了密钥，节点也无法验证。这种情况下，整个集群将无法恢复，必须重新初始化和搭建( `docker swarm init --force-new-cluster` )，当然此时或许会用上之前不推荐的 manager 节点强制退群操作也说不定。这就是为什么启用自动锁定功能非常重要，以确保在节点重启时能够安全地管理密钥。
