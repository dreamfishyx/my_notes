...
2. vim 使用系统剪贴板，运行 `vim --version | grep clipboard` 查看是否支持，`-clipboard` 表示不支持直接访问系统剪贴板。
...
3. 编辑`/usr/share/vim/vim91/defaults.vim`(`vim91`文件夹名称可能略有不同)，添加 `set clipboard=unnamed`使 vim 默认使用系统剪贴板:
