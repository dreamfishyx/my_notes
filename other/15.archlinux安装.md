...
15. docker:
    ...
    4. 配置镜像加速:编辑文件`sudo vim /etc/docker/daemon.json`配置清华镜像加速
...
4. 换源和安装yay：
   ...
   2. `vim /etc/pacman.d/mirrorlist`添加如下源：
   ...
   3. 开启32位支持库与ArchLinux中文社区仓库archlinuxcn。编辑`vim /etc/pacman.conf`：
...
9. 基本软件安装：`pacman -S base-devel`

> 1. wsl:检测到localhost代理配置，但未镜像到WSL。NAT模式下的WSL不支持localhost代理。C:\Users\<your_username>目录下创建一个.wslconfig文件，然后在文件中写入如下内容:
>
>    ```bash
>    [experimental]
