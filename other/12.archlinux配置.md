...
2. java: 
   ...
   > 其实也可安装 openjdk ,即运行 `sudo pacman -S jdk21-openjdk`。若是安装多个版本的 Java，可以使用 `archlinux-java` 工具设置默认版本：
   >
   > ```bash
...
3. annaconda：[清华镜像](https://mirrors.tuna.tsinghua.edu.cn/anaconda/archive/),~~实际上若是linux官方存在自己的软件发行版，最好安装官方的，适配要好一点。~~
   ...
   > (建议采用这种，上面安装方式 anaconda 使用 yay 更新会很烦)此外，可以直接在官网或者镜像站下载安装脚本进行安装([清华镜像](https://mirrors.tuna.tsinghua.edu.cn/anaconda/archive/?C=M&O=D))(可以使用 wget 下载):我这里在清华镜像下载 linux-x86-64 格式。<br><img src="./assets/image-20241204015025400.png" alt="image-20241204015025400" style="zoom:67%;" />
   >
...
4. mysql：[参考](https://blog.csdn.net/qq_40829735/article/details/120825048)(不安装，开发用 docker 将就一下)
...
5. maven:
   ...
   2. 配置环境：`sudo vim /etc/profile`，配置完成加载配置文件`source /etc/profile`运行`mvn-v`测试
   ...
   3. 修改maven配置：`sudo vim /usr/share/java/maven/conf/settings.xml `
...
15. docker:
    ...
    5. 配置镜像加速:编辑文件(可能需要先创建文件夹)`sudo mkdir -p /etc/docker && sudo vim /etc/docker/daemon.json`配置清华镜像加速
...
16. thefuck(一个命令行纠错工具):`sudo pacman -S thefuck`,然后编辑`vim ~/.zshrc` 添加:
...
16. tldr(一个使用手册):~~`sudo pacman -S nodejs npm && npm install -g tldr`~~，~~使用 npm 安装的似乎无法使用环境变量配置语言，改用` pip install tldr` 安装~~


    1. 然后编辑`vim ~/.zshrc` 添加如下配置后`source ~/.bashrc`:
    ...
    1. 更新本地缓存:`tldr --update`
    
    > 上述安装方式都不建议，改用`sudo pacman -S tealdeer && tldr --update `,环境配置中语言选项不生效,改用`alias how="tldr -L zh"`(建议,也可以定义一个函数`how() { tldr "$@" -L zh }`)或者`alias tldr="tldr -L zh"`(不建议)实现中文输入。此外根据[官方文档](https://github.com/tldr-pages/tldr/blob/main/CLIENT-SPECIFICATION.md#command-line-interface)，似乎添加`LANG` 和 `LANGUAGE`变量也能实现(或者修改`/etc/default/locale` 文件)，但是这可能会对其他软件或者系统语言产生一定影响。
...
5. 其实对于windows也可以这样配置:

   1. 打开文件夹 `C:\Users\[User]\.ssh` 创建文件 `config` (无文件扩展名)
...
1. 如果你希望让 Linux 使用本地时间，以便与 Windows 保持一致，你需要修改 Linux 的 `timedatectl` 设置。

   1. 你可以检查当前的硬件时钟是否使用 UTC：

      ```bash
      $ timedatectl
      
                     Local time: 一 2025-01-06 09:57:18 CST
                 Universal time: 一 2025-01-06 01:57:18 UTC
                       RTC time: 一 2025-01-06 01:57:18
   ...
   2. **将硬件时钟设置为本地时间**： 运行以下命令将硬件时钟设置为本地时间：

      ```bash
      sudo timedatectl set-local-rtc 1
      ```

   3. 验证设置： 运行 timedatectl，你应该看到 RTC in local TZ，表示硬件时钟正在使用本地时间。
...
2. `prime-run` 部分应用可以不需要安装 optimus-manager，直接运行。但不推荐这样，因为除了部分应用会报错之外也无法使用仅独立显卡模式。
   ...
   1. 通过以下命令安装 nvidia-prime 包：`sudo pacman -S nvidia-prime`
      ...
      > `yay -S optimus-manager optimus-manager-qt` 也安装上,在hyprland无法使用，官方建议删了。
...
> - archlinux 官方提供对应文件:[官方教程](https://wiki.archlinuxcn.org/wiki/%E5%9C%A8_WSL_%E4%B8%8A%E5%AE%89%E8%A3%85_Arch_Linux)
> - 在配置 wsl 之前，确保仔细阅读以下文档:
>   - [使用 WSL 访问网络应用程序](https://learn.microsoft.com/zh-cn/windows/wsl/networking)
>   - [WSL中的高级设置配置:wslconfig](https://learn.microsoft.com/zh-cn/windows/wsl/wsl-config#wslconfig)
>   - [WSL中的高级设置配置:wslconf](https://learn.microsoft.com/zh-cn/windows/wsl/wsl-config#wslconf)
...
5. 基本软件安装：`sudo pacman -S base base-devel git curl wget neofetch net-tools dnsutils inetutils`
...
6. 创建新用户：
   ...
   4. 退出 wsl，设置启动时默认用户:`./Arch.exe config --default-user {username}`
...
9. 显示信息：`pacman -S fastfetch`

   1. 生成默认配置文件:` fastfetch --gen-config`

   2. 修改配置文件:`~/fastfetch/config.jsonc`

      > - 安装`sudo pacman -S lolcat`
      > - 运行`fastfetch | lolcat`

10. wsl:检测到localhost代理配置，但未镜像到WSL。NAT模式下的WSL不支持localhost代理。`C:\Users\<your_username>` 目录下创建一个 `.wslconfig` 文件，然后在文件中写入如下内容:[WSL中的高级设置配置(wslconfig)](https://learn.microsoft.com/zh-cn/windows/wsl/wsl-config#wslconfig)
    ...
    ```bash
    [wsl2]
    # 启用 WSL2 的 GUI 应用程序支持（需要 Windows 11 或 WSLg）
    guiApplications=true
    # 网络模式："mirrored" 会镜像 Windows 的网络栈到 WSL2
    networkingMode=mirrored
    # 通过 Windows 进行 DNS 解析（解决 VPN 环境下的 DNS 问题）
    dnsTunneling=true
    # 应用 Windows 防火墙规则到 WSL2
    firewall=true
    # 自动同步 Windows 代理设置到 WSL2
    autoProxy=true
    
    [experimental]
    # 自动内存回收模式："gradual" 表示渐进式回收（默认），"dropcache" 为强制回收
    autoMemoryReclaim=gradual  
    # 使用稀疏 VHD 节省磁盘空间（需要 WSL 0.67.6+）
    sparseVhd=true
    # mirrored模式下允许通过分配给主机的IP地址连接到主机或允许主机通过该地址连接到wsl(localhost仍可以)
    hostAddressLoopback=true
    ```
...
13. 配置几个命令别名`vim ~/.zshrc`编辑文件：
    ...
    # 一些临时配置,例如快速进入项目目录等，定期修改(其实也可以将自定义配置全放进该文件)
    source ~/env.tmp
...
15. 无法使用 GUI app解决方式:[issues](https://github.com/microsoft/WSL/issues/8596)

    ```conf
    # Put in ~/.config/user-tmpfiles.d/wayland.conf
    # Then systemctl --user enable --now systemd-tmpfiles-setup.service systemd-tmpfiles-clean.timer
    # https://github.com/microsoft/wslg/issues/1032#issuecomment-2458021984
    # https://github.com/yuk7/ArchWSL/issues/357#issuecomment-2062677233
    ...
    ```conf
    # Put in /etc/tmpfiles.d/x11.conf
    # https://github.com/microsoft/WSL/issues/6999#issuecomment-2303010704
    # Type Path  Mode User Group Age Argument
    L+ /tmp/.X11-unix - - - - /mnt/wslg/.X11-unix
    ```

16. 一些wsl配置:`sudo vim /etc/wsl.conf` ,下面配置大部分都是默认值(不用配)，同时不建议修改(尤其是 root )(修改前请确保你知道你在干啥):[WSL 中的高级设置配置(wslconf)](https://learn.microsoft.com/zh-cn/windows/wsl/wsl-config#wslconf)
...
> 目前 wsl2 已经支持 GUI 程序 和在 wsl 中使用 windows 程序。
...
1. https://blog.csdn.net/m0_62721116/article/details/135187285
2. https://zhuanlan.zhihu.com/p/14300263211
3. https://wzk0.github.io/gne/
4. https://www.bilibili.com/opus/673745116107636755
