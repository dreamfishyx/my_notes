...
>
> https://github.com/flickowoa/dotfiles/tree/master?tab=readme-ov-file
>
...
3. 本人系统存在双GPU ( intel 和 navidia ),参照官方文档，对于 Nvidia，参照[官方文档](https://wiki.hyprland.org/Nvidia/)安装一些必要驱动软件
   ...
   2. 对于 nividia 配置:
      ...
      > 对于双显卡,我不知道是否应该配置 hyprland 中关于 nividia 的相关内容([wiki](https://wiki.hyprland.org/Nvidia/)),目前位置一直使用 intel 。后续我尝试配置该部分,在执行 `sudo mkinitcpio -P` 时报错 `nvidia nvidia_modeset nvidia_uvm nvidia_drm` 没有找到: 我没有安装安装内核头文件: 运行 `sudo pacman -S linux-headers` 安装并执行 `pacman -Q linux linux-headers` 确认内核头文件版本是否匹配。完成后,重新执行 `sudo mkinitcpio -P` 更新 initramfs 镜像。
   ...
   4. 一些 cpu 数据显示工具:
      ...
      2. btop:直观且美观的终端工具，显示 CPU、内存、磁盘、网络和部分 GPU 的使用情况。
         ...
         cp -r ./btop/themes/*  /usr/share/btop/themes/ ( 不建议安装在 ~/.config/btop/themes/ )  
...
https://wiki.archlinux.org/title/HiDPI#Xwayland

https://wiki.archlinux.org/title/HiDPI#Xwayland_2
...
如果手动设置屏幕系数，请务必设置`QT_AUTO_SCREEN_SCALE_FACTOR=0`和`QT_ENABLE_HIGHDPI_SCALING=0` ，否则某些明确强制启用高 DPI 的应用程序会缩放两次。
...
6. 关于 kitty 的初始化大小问题,找到一个解决方法[待定](https://www.reddit.com/r/hyprland/comments/1gx44b7/how_to_set_kitty_have_a_specific_window_size_when/)
...
https://www.gnome-look.org/p/2206122/



编辑 GRUB 配置文件 /etc/default/grub，添加或修改以下行：

GRUB_THEME=/boot/grub/themes/theme-name/theme.txt



ll /usr/share/grub/themes 





将主题文件复制到 GRUB 的主题目录，例如 /boot/grub/themes/：
...
出现这种情况，是因为系统中的 pulseaudio-bluetooth 依赖了 pulseaudio，导致在尝试删除 pulseaudio 时产生冲突。要解决这个问题，你可以分以下步骤进行操作。
解决步骤

1. 确保 PipeWire 包已安装

在切换到 PipeWire 前，先安装 PipeWire 及其相关组件：

sudo pacman -S pipewire pipewire-pulse pipewire-alsa pipewire-jack wireplumber

2. 替换 pulseaudio-bluetooth 为 PipeWire 版本

PipeWire 提供了蓝牙支持包 pipewire-pulse 和 pipewire-media-session，它们可以代替 pulseaudio-bluetooth。

先移除 pulseaudio-bluetooth：

sudo pacman -Rdd pulseaudio-bluetooth

然后安装 PipeWire 的蓝牙支持包：

sudo pacman -S pipewire-pulse

3. 删除 pulseaudio

现在可以安全地移除 pulseaudio：
...
使用以下命令确认 PipeWire 是否正常运行：

pactl info

如果输出中 Server Name 为 PipeWire，说明切换成功。
注意事项

    如果使用蓝牙设备，请确保已经安装 bluez 和 bluez-utils，并且蓝牙服务已启动。
    若有特定应用程序仍依赖 PulseAudio，请检查是否可以通过 pipewire-pulse 兼容。
