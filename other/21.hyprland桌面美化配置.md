...
7. 安装一些W ayland 环境下运行图形应用所需的组件，确保应用程序能够在 Wayland 显示服务器上正确渲染和交互。
   ...
   > qt5ct 是一个用来配置 Qt5 应用程序外观和行为的工具。在使用 Wayland 和 X11 时，特别是对于使用 Qt 的应用程序，qt5ct 可以设置应用程序的主题、字体、图标以及其他一些界面元素的样式。
...
3. 环境配置：`sudo vim /etc/environment`
...
9. 运行`source ~/.zshrc`重新加载配置文件。

10. 

11. oh-my-zsh 内置了 z 插件。z 是一个文件夹快捷跳转插件，对于曾经跳转过的目录，只需要输入最终目标文件夹名称，就可以快速跳转，避免再输入长串路径，提高切换文件夹的效率

12. oh-my-zsh 内置了 extract 插件。extract 用于解压任何压缩文件，不必根据压缩文件的后缀名来记忆压缩软件。使用 x 命令即可解压文件，效果如下
...
首先，考虑到你当前的缩放比例（scale 为 1.5），系统已经在视觉上放大了显示内容。为了使字体和 UI 元素的大小适中，我们可以调整 DPI 设置，以便在 1.5x 缩放的情况下，保持舒适的阅读和使用体验。
1. 计算合适的 DPI

    标准 DPI：通常，标准的 DPI 是 96，这是大多数显示器的默认设置，适用于大部分低分辨率显示器。
    1.5x 缩放：由于你已经设置了 1.50 的缩放比例，你的系统会按放大比例渲染内容。因此，DPI 设置需要适当调整，以补偿这种缩放。

为了匹配 1.5 倍缩放，可以按照以下方式调整 DPI：

    原始 DPI 96 乘以缩放比例 1.5，得到：
    
    96 * 1.5 = 144
    
    这样，如果你想让字体和 UI 元素在 1.5x 缩放下显得更加合适，可以将 DPI 设置为 144。

2. 修改 DPI 设置

你可以通过以下命令来调整 DPI 设置，使其符合你的缩放需求：

exec-once = echo 'Xft.dpi: 144' | xrdb -merge

如果你觉得 DPI 144 还是偏大，或者不适合你当前的显示需求，可以尝试略微调整：

    DPI 120：如果你觉得 144 DPI 太大，可以尝试设置为 120 DPI，这样元素会更小一点。
    DPI 160：如果你希望界面元素更加突出和易于阅读，可以设置为 160 DPI。
