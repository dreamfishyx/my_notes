...
对的，如果你先配置了 `all_proxy`，然后再单独设置 `https_proxy`，在某些情况下（取决于程序和工具的行为），`https_proxy` 会优先使用，因为它是一个专门为 HTTPS 流量设置的环境变量，而 `all_proxy` 是一个通用的代理设置。
...
```
bash复制代码export http_proxy="http://your.proxy.address:port"
export https_proxy="https://your.proxy.address:port"
export socks_proxy="socks5://your.proxy.address:port"
export no_proxy="localhost,127.0.0.1,*.local"
```



如果你想禁用代理，可以通过取消环境变量的设置来实现：

```
bash复制代码unset http_proxy
unset https_proxy
unset socks_proxy
unset no_proxy
```
...
大多数现代命令行工具和应用程序会自动读取环境变量并根据它们来配置代理，但有些程序可能需要额外的配置或不同的设置方法。
