...
3. 本人系统存在双GPU ( intel 和 navidia ),参照官方文档，对于 Nvidia，参照[官方文档](https://wiki.hyprland.org/Nvidia/)安装一些必要驱动软件
   ...
   1. 对于 intel 显卡:http://www.qinsibo.com/2019/04/17/KDE%E6%98%BE%E7%A4%BA%E9%97%AA%E7%83%81%E9%97%AE%E9%A2%98/
      ...
      > GRUB_CMDLINE_LINUX_DEFAULT="loglevel=5 nowatchdog nvidia_drm.modeset=1 i915.enable_psr=0"

      在 Linux 系统中，可以通过设置 `vblank_mode` 环境变量来控制垂直同步行为：

      - **`vblank_mode=0`**：禁用 V-Sync，不进行帧数同步，可能导致撕裂。
      - **`vblank_mode=1`**：启用 V-Sync，显卡会与显示器刷新率同步，避免撕裂。
      - **`vblank_mode=2`**：仅当显卡的渲染帧率超过显示器刷新率时才启用 V-Sync。

      通过这种方式，你可以根据个人需求选择是否启用垂直同步。

      垂直同步（V-Sync）是解决图像撕裂问题的一种技术，通过同步显卡的渲染帧率和显示器的刷新率来提供更加平滑的视觉体验。启用 V-Sync 可以有效避免撕裂，但在某些情况下可能会增加输入延迟或影响性能。如果你的系统经常遇到图像撕裂问题，可以尝试启用垂直同步来改善体验。

      #### 3. **V-Sync 对性能的影响**

      启用 V-Sync 可以消除撕裂，但有时也可能影响游戏或应用的性能，特别是在显卡性能不足时，因为显卡会等待显示器准备好再渲染下一帧。如果显卡的帧率不稳定，启用 V-Sync 可能导致输入延迟或帧数下降。
...
2. 下载主题：
   ...
   5. 主题: Sugar Candy for SDDM](https://store.kde.org/p/1312658/)
...
4. sddm 安装后，可能会卡死，此时则需要安装 `sudo pacman -S xf86-video-intel` 和 `sudo pacman -S xf86-video-nouveau` 。
...
3. 配置 zsh 和 powerlevel10k:
   ...
   3. 装完后使用 `vim ~/.zshrc`配置好 `ZSH_THEME="powerlevel10k/powerlevel10k"`，运行`p10k configure`配置自定义设置。
...
8. 修改 hyprland 配置
   ...
   3. 修改主题边框、窗口间隔:` vim ~/.config/hypr/themes/theme.conf`
...
13. clash dev rev:
    ...
    2. 配置启动服务: clash verge rev 的设置里有启动脚本脚本这一选项,我猜可以用来配置环境变量,但是似乎无法在退出时删除环境变量。所以还是以服务的形式启动。
